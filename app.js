...
function showScreen(screenId) {
  ...
  if (screenId === 'newTripScreen') renderNewTripDrivers();
...
function closeModal(id) {
  ...
  if (id === 'mapModal' && state.mapInstance) {
    state.mapMode = null;
  }
}
...
function renderNewTripDrivers() {
  const select = $('#tripDriverSelect');
  if (!select) return;

  const drivers = state.config.drivers || [];
  if (!drivers.length) {
    select.innerHTML = '<option value="">Nenhum cadastro encontrado. Cadastre na Área do Administrador.</option>';
  ...
  select.innerHTML = drivers.map((driver) => `
    <option value="${driver.id}">${escapeHtml(driver.name)} • Van ${escapeHtml(driver.vanNumber)}</option>
  `).join('');
}

function getSelectedDriverForTrip() {
  const select = $('#tripDriverSelect');
  const driverId = select?.value;
  const driver = (state.config.drivers || []).find((item) => item.id === driverId);

  if (!driver) {
    showToast('Cadastre e selecione a pessoa/van antes de iniciar a viagem.');
    return null;
  }
...
function createTrip(direction) {
  const driver = getSelectedDriverForTrip();
  if (!driver) return;
  ...
  logTrip('inicio_viagem', `Viagem iniciada no sentido ${directionLabel(direction)} pela van ${driver.vanNumber}.`, { direction, driverId: driver.id, driverName: driver.name, vanNumber: driver.vanNumber });
  saveCurrentTrip();
  startGeolocation();
...
function renderHome() {
  ...
  activeCard.innerHTML = `
    ...
    <p>Van ${escapeHtml(state.currentTrip.vanNumber || '-')} • ${escapeHtml(state.currentTrip.driverName || 'Sem cadastro')} • Início: ${formatDateTime(state.currentTrip.startedAt)}</p>
...
function renderTrip() {
  ...
  $('#tripStatus').textContent = `Van ${trip.vanNumber || '-'} • ${trip.driverName || 'Sem cadastro'} • Início: ${formatDateTime(trip.startedAt)} • Tarifa inteira ${money(trip.fareFullCents)} • Luso ${money(trip.fareLusoCents)}`;
...
function startGeolocation() {
  ...
  state.geoWatchId = navigator.geolocation.watchPosition(
    (position) => {
      updateLastLocationFromPosition(position, 'gps');
      renderGpsStatus();
...
function updateLastLocationFromPosition(position, source = 'gps') {
  const loc = {
  ...
  if (state.currentTrip) {
    state.currentTrip.lastLocation = loc;
    state.currentTrip.locationLog = Array.isArray(state.currentTrip.locationLog) ? state.currentTrip.locationLog : [];

    const lastLog = state.currentTrip.locationLog.at(-1);
    const shouldLog = !lastLog || source !== 'gps' || (Date.now() - new Date(lastLog.at).getTime()) > 30000;
    if (shouldLog) {
      state.currentTrip.locationLog.push(loc);
      state.currentTrip.locationLog = state.currentTrip.locationLog.slice(-300);
      logTrip('localizacao_atualizada', `Localização da van ${state.currentTrip.vanNumber || ''} atualizada.`, { lat: loc.lat, lng: loc.lng, accuracy: loc.accuracy, source });
    } else {
      saveCurrentTrip();
    }
  }
...
function refreshGps() {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      updateLastLocationFromPosition(position, 'gps_manual');
      renderGpsStatus();
      checkRouteTriggers(false);
...
function renderGpsStatus(customMessage = '') {
  ...
  const source = state.lastLocation.source === 'simulacao' ? 'simulado' : state.lastLocation.source === 'gps_manual' ? 'manual' : 'ao vivo';
  box.innerHTML = `GPS ${source}: ${state.lastLocation.lat.toFixed(6)}, ${state.lastLocation.lng.toFixed(6)} • precisão aprox. ${Math.round(state.lastLocation.accuracy || 0)}m • ${formatDateTime(state.lastLocation.at)}`;
}
...
function vibrateAlarm() {
  if ('vibrate' in navigator) {
    navigator.vibrate([650, 180, 650, 180, 900]);
  }
}
...
function openPhotoAlert(pointId, alertId = null) {
  ...
  $('#photoModalTitle').textContent = `Foto obrigatória: ${point.name}`;
  $('#photoModalText').textContent = `ATENÇÃO: você chegou ao ponto ${point.name}. Tire a foto obrigatória agora.`;
...
function historyItemHtml(trip) {
  ...
  return `
    <div class="list-item">
      <header>
        <div>
          <h3>${formatDate(trip.startedAt)} • ${directionLabel(trip.direction)} • Van ${escapeHtml(trip.vanNumber || '-')}</h3>
  ...
  box.innerHTML = `
    <section class="report-section">
      ...
      <table class="report-table">
        ...
        <tr><th>Pessoa / cobrador</th><td>${escapeHtml(trip.driverName || '-')}</td></tr>
        <tr><th>Número da van</th><td>${escapeHtml(trip.vanNumber || '-')}</td></tr>
...
function saveFares() {
  ...
  saveConfig();
  showToast('Valores salvos. Novas viagens usarão essas tarifas e a duração média salva.');
}
...
function saveDriver() {
  ...
  const duplicatedVan = (state.config.drivers || []).some((driver) => driver.vanNumber.toLowerCase() === vanNumber.toLowerCase() && driver.id !== id);
  if (duplicatedVan && !confirm('Já existe cadastro com esse número de van. Deseja salvar mesmo assim?')) return;
...
function removeDriver(id) {
  const driver = state.config.drivers.find((item) => item.id === id);
  if (!driver) return;
  const hasTrips = state.currentTrip?.driverId === id || state.history.some((trip) => trip.driverId === id);
  const message = hasTrips
    ? `Remover ${driver.name}? Viagens antigas continuarão com nome e van gravados no relatório.`
    : `Remover ${driver.name}?`;
...
function renderDriversList() {
  ...
  const drivers = state.config.drivers || [];
  if (!drivers.length) {
    list.innerHTML = '<p class="muted">Nenhuma pessoa/van cadastrada ainda.</p>';
    return;
  }
  ...
  list.innerHTML = drivers.map((driver) => `
    <div class="list-item">
      <header>
        <div>
          <h3>${escapeHtml(driver.name)} • Van ${escapeHtml(driver.vanNumber)}</h3>
          <p>${driver.note ? escapeHtml(driver.note) : 'Sem observação'} • atualizado em ${formatDateTime(driver.updatedAt)}</p>
        </div>
      </header>
...
function useCurrentLocationFor(target) {
  if (!state.lastLocation) {
    showToast('Atualize o GPS primeiro ou permita localização no navegador.');
    return;
  }
  ...
  if (target === 'point') {
    ...
    showToast('Coordenadas do ponto preenchidas com o GPS atual.');
  }

  if (target === 'luso') {
    $('#lusoLatInput').value = state.lastLocation.lat.toFixed(7);
    $('#lusoLngInput').value = state.lastLocation.lng.toFixed(7);
    showToast('Coordenadas do Luso preenchidas com o GPS atual.');
  }
}

function openMapPicker(mode) {
  state.mapMode = mode;
  const isLuso = mode === 'luso';
  $('#mapModalTitle').textContent = isLuso ? 'Escolher ponto Luso no mapa' : 'Escolher ponto obrigatório no mapa';

  const latInput = isLuso ? $('#lusoLatInput') : $('#pointLatInput');
  const lngInput = isLuso ? $('#lusoLngInput') : $('#pointLngInput');
  const initialLat = Number(latInput.value) || Number(state.lastLocation?.lat) || -12.9714;
  const initialLng = Number(lngInput.value) || Number(state.lastLocation?.lng) || -38.5014;

  state.mapTempLocation = { lat: initialLat, lng: initialLng };
  $('#mapPickerStatus').textContent = `Alfinete: ${initialLat.toFixed(6)}, ${initialLng.toFixed(6)}`;
  openModal('mapModal');

  if (!window.L) {
    $('#mapPicker').innerHTML = '<div class="gps-box">Mapa indisponível porque a biblioteca Leaflet não carregou. Verifique a internet ou use o GPS atual.</div>';
    return;
  }

  setTimeout(() => {
    if (!state.mapInstance) {
      state.mapInstance = L.map('mapPicker');
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; OpenStreetMap'
      }).addTo(state.mapInstance);
      state.mapInstance.on('click', (event) => setMapMarker(event.latlng.lat, event.latlng.lng));
    }

    state.mapInstance.setView([initialLat, initialLng], 16);
...
function setMapMarker(lat, lng) {
  state.mapTempLocation = { lat, lng };
  if (state.mapInstance && window.L) {
    if (!state.mapMarker) state.mapMarker = L.marker([lat, lng], { draggable: true }).addTo(state.mapInstance);
    else state.mapMarker.setLatLng([lat, lng]);

    state.mapMarker.off('dragend');
    state.mapMarker.on('dragend', () => {
      const pos = state.mapMarker.getLatLng();
      setMapMarker(pos.lat, pos.lng);
    });
  }
  $('#mapPickerStatus').textContent = `Alfinete selecionado: ${lat.toFixed(7)}, ${lng.toFixed(7)}`;
}

function mapUseCurrentLocation() {
  if (!state.lastLocation) {
    showToast('GPS atual ainda não disponível. Toque em Atualizar GPS na viagem ou permita localização.');
    return;
  }
  setMapMarker(state.lastLocation.lat, state.lastLocation.lng);
  if (state.mapInstance) state.mapInstance.setView([state.lastLocation.lat, state.lastLocation.lng], 17);
}

function confirmMapPoint() {
  if (!state.mapMode || !state.mapTempLocation) {
    showToast('Escolha um ponto no mapa primeiro.');
    return;
  }

  const { lat, lng } = state.mapTempLocation;
  if (state.mapMode === 'luso') {
...
function latestTripForDriver(driver) {
  const trips = [];
  if (state.currentTrip && (state.currentTrip.driverId === driver.id || state.currentTrip.vanNumber === driver.vanNumber)) trips.push(state.currentTrip);
  trips.push(...state.history.filter((trip) => trip.driverId === driver.id || trip.vanNumber === driver.vanNumber));
  return trips.sort((a, b) => new Date(b.startedAt) - new Date(a.startedAt))[0] || null;
}

function estimateTripPosition(trip) {
  if (!trip) return { status: 'offline', label: 'Sem viagem registrada', details: 'Ainda não há dados para esta van.', lat: null, lng: null };

  const lastReal = trip.lastLocation || trip.locationLog?.at?.(-1);
  if (lastReal?.lat && lastReal?.lng) {
    return {
      status: trip.endedAt ? 'offline' : 'real',
      label: trip.endedAt ? 'Última localização conhecida' : 'GPS real',
      details: `${formatDateTime(lastReal.at)} • precisão aprox. ${Math.round(lastReal.accuracy || 0)}m`,
      lat: Number(lastReal.lat),
      lng: Number(lastReal.lng)
    };
  }

  const latestPhoto = [...(trip.photos || [])].reverse().find((photo) => Number.isFinite(Number(photo.lat)) && Number.isFinite(Number(photo.lng)));
  if (latestPhoto) {
    return {
      status: 'estimated',
      label: 'Estimado pela última foto',
      details: `${latestPhoto.pointName} • ${formatDateTime(latestPhoto.capturedAt)}`,
      lat: Number(latestPhoto.lat),
      lng: Number(latestPhoto.lng)
    };
  }

  const latestAlert = [...(trip.photoAlerts || [])].reverse().find((alert) => Number.isFinite(Number(alert.lat)) && Number.isFinite(Number(alert.lng)));
  if (latestAlert) {
    return {
      status: 'estimated',
      label: 'Estimado pelo último alerta de ponto',
      details: `${latestAlert.pointName} • ${formatDateTime(latestAlert.generatedAt)}`,
      lat: Number(latestAlert.lat),
      lng: Number(latestAlert.lng)
    };
  }

  const routePoints = (state.config.photoPoints || [])
    .filter((point) => point.direction === 'both' || point.direction === trip.direction)
    .filter((point) => Number.isFinite(Number(point.lat)) && Number.isFinite(Number(point.lng)))
    .sort((a, b) => Number(a.order || 0) - Number(b.order || 0));

  if (routePoints.length) {
    const elapsedMinutes = Math.max(0, (Date.now() - new Date(trip.startedAt).getTime()) / 60000);
    const duration = Math.max(5, Number(state.config.routeDurationMinutes || 60));
    const progress = Math.min(1, elapsedMinutes / duration);
    const index = Math.min(routePoints.length - 1, Math.round(progress * (routePoints.length - 1)));
    const point = routePoints[index];
    return {
      status: 'estimated',
      label: 'Estimado pela rota e tempo',
      details: `Aprox. perto de ${point.name}. ${Math.round(progress * 100)}% da duração média (${duration} min).`,
      lat: Number(point.lat),
      lng: Number(point.lng)
    };
  }

  return { status: 'offline', label: 'Sem localização', details: 'Sem GPS, foto, alerta ou pontos de rota suficientes para estimar.', lat: null, lng: null };
}
...
function renderVanLocationsPanel() {
  ...
  if (!drivers.length) {
    list.innerHTML = '<p class="muted">Cadastre pelo menos uma pessoa/van para aparecer no painel.</p>';
    return;
  }

  list.innerHTML = drivers.map((driver) => {
    const trip = latestTripForDriver(driver);
    const position = estimateTripPosition(trip);
    const mapsUrl = position.lat && position.lng ? `https://www.google.com/maps?q=${position.lat},${position.lng}` : '';
    return `
      <div class="list-item fleet-card ${position.status}">
        <header>
          <div>
            <h3>Van ${escapeHtml(driver.vanNumber)} • ${escapeHtml(driver.name)}</h3>
            <p>${trip ? `${trip.endedAt ? 'Última viagem encerrada' : 'Em viagem'} • ${directionLabel(trip.direction)} • início ${formatDateTime(trip.startedAt)}` : 'Sem viagem registrada'}</p>
          </div>
          <span class="badge ${position.status === 'real' ? 'ok' : position.status === 'estimated' ? 'warn' : 'danger'}">${position.label}</span>
        </header>
        <p>${escapeHtml(position.details)}</p>
        ${position.lat && position.lng ? `<p class="location-line">${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}</p>` : ''}
        ${mapsUrl ? `<a class="btn small ghost" href="${mapsUrl}" target="_blank" rel="noopener">Abrir no mapa</a>` : ''}
      </div>
    `;
  }).join('');
}
...
function simulatePoint(id) {
  ...
  state.lastLocation = { lat: Number(point.lat), lng: Number(point.lng), accuracy: 5, at: nowIso(), simulated: true, source: 'simulacao' };
  if (state.currentTrip) {
    state.currentTrip.lastLocation = state.lastLocation;
    state.currentTrip.locationLog = Array.isArray(state.currentTrip.locationLog) ? state.currentTrip.locationLog : [];
    state.currentTrip.locationLog.push(state.lastLocation);
    saveCurrentTrip();
  }
  renderGpsStatus();
  ...
  list.innerHTML = allTrips.map((trip) => {
    ...
    return `
      <div class="list-item">
        <header>
          <div>
            <h3>${trip.endedAt ? 'Encerrada' : 'Em andamento'} • Van ${escapeHtml(trip.vanNumber || '-')} • ${formatDateTime(trip.startedAt)}</h3>
            <p>${escapeHtml(trip.driverName || '-')} • ${directionLabel(trip.direction)} • ${totals.totalPassengers} passageiros • ${money(totals.expectedCents)} esperado • ${totals.photosPending} pendente(s) • ${totals.photosSuspicious} suspeita(s)</p>
          </div>
        </header>
...
function bindEvents() {
  ...
  $('#btnMapUseCurrent').addEventListener('click', mapUseCurrentLocation);
  $('#btnConfirmMapPoint').addEventListener('click', confirmMapPoint);
  $('#btnRefreshFleetPanel').addEventListener('click', renderVanLocationsPanel);
}
