...
function renderVanLocationsPanel() {
  ...
  list.innerHTML = rows.map(({ driver, trip, position }) => {
    ...
    const mapsUrl = hasPos ? `https://www.google.com/maps/search/?api=1&query=${position.lat},${position.lng}` : '';
    return `
      <div class="list-item ${sourceClass}">
        <header>
          <div>
            ...
            ${hasPos ? `<p>${position.lat.toFixed(6)}, ${position.lng.toFixed(6)} • <a class="map-link" href="${mapsUrl}" target="_blank" rel="noopener">Abrir no Google Maps</a></p>` : ''}
          </div>
...
function renderLeafletFleetMap(rows) {
  ...
  if (typeof L === 'undefined') {
    status.innerHTML = 'Mapa visual indisponível porque o Leaflet não carregou. Os links do Google Maps continuam disponíveis nas vans com coordenada.';
    return;
  }
  ...
  rows.forEach(({ driver, trip, position }) => {
    ...
    bounds.push(latLng);
    const gmaps = `https://www.google.com/maps/search/?api=1&query=${position.lat},${position.lng}`;
    const marker = L.marker(latLng).addTo(state.fleetMap);
    marker.bindPopup(`
      <div class="van-popup">
        ...
        <a href="${gmaps}" target="_blank" rel="noopener">Abrir no Google Maps</a>
      </div>
    `);
    state.fleetMarkers.push(marker);
  });
  state.fleetMap.fitBounds(bounds, { padding: [28, 28], maxZoom: 16 });
  status.textContent = `${rows.length} van(s) com alfinete no mapa. Base visual: OpenStreetMap; cada alfinete abre também no Google Maps.`;
}
...
async function renderGoogleFleetMap(rows) {
  ...
  status.textContent = `${rows.length} van(s) com alfinete no Google Maps. Atenção: em app local, a chave fica visível no navegador; em produção, proteja a chave no servidor.`;
}
...
function bindEvents() {
  ...
  $('#btnRefreshFleetPanel').addEventListener('click', renderFleetPanel);
}
